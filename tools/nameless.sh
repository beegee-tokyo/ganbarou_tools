...
if [ $? -eq 0 ]; then
	...
	if [ $trgt == "p4" ] || [ $trgt == "p4wifi" ]; then
		...
		echo -e $CL_MAG"=============================================="$CL_RST
		OTAPACKAGE="$OUT/nameless-4.4.2-$NOWORG-$OLD_DEVICE-HOMEMADE.zip"
		mkdir $REPACK
		mkdir $REPACK/ota1
		cd $REPACK/ota1
		...
		$REPACK/update1.zip $OUT/nameless-4.4.2-$NOWORG-$OLD_DEVICE1-HOMEMADE.zip
		echo -e $CL_GRN"============================================"$CL_RST
		echo -e $CL_GRN"Create a md5 checksum image of the repacked $NEW_DEVICE1 package"
		echo -e $CL_GRN"============================================"$CL_RST
		(
		img=`basename $OUT/nameless-4.4.2-$NOWORG-$OLD_DEVICE1-HOMEMADE.zip`
		cd `dirname $OUT/nameless-4.4.2-$NOWORG-$OLD_DEVICE1-HOMEMADE.zip`
		$MD5 $img >$img.md5sum
		)
		...
		echo -e $CL_CYN"md5:"$CL_MAG" $(cat $OUT/nameless-4.4.2-$NOWORG-$OLD_DEVICE1-HOMEMADE.zip.md5sum | awk '{ print $1 }')"$CL_RST
